<template>
  <div class="ptcg-list-grid">
    <div v-for="s in series" :key="s.id" class="ptcg-list-card" @click="$emit('select', s)">
      <img v-if="s.logo" :src="s.logo" :alt="s.name" class="ptcg-list-logo" loading="lazy" />
...
defineProps<{ series: { id: string; name: string; logo?: string }[] }>()
defineEmits<{ select: [serie: { id: string; name: string }] }>()
</script>
